...
MatS elem2node ( const MatS &conn );

// list with DOF-numbers in sequential order
MatS dofs ( size_t nnode , size_t ndim );

// renumber list of indices
// - renumber to lowest possible numbers (e.g. [0,3,4,2] -> [0,2,3,1])
MatS renumber ( const MatS &in );
// - renumber to begin [ idx , ... ] or end [ ... , idx ] (e.g. [0,1,3,2] -> [3,0,2,1]; with idx=0)
MatS renumber ( const MatS &in , const ColS &idx , std::string location="end" );
...
MatS renumber ( const MatS &in , const ColS &idx , std::string loc )
{
