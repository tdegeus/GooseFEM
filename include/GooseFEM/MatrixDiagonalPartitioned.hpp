        ...
        xt::xtensor<double,2> &b) const
{
  ...
  for (size_t m = 0 ; m < m_nnode ; ++m) {
    for (size_t i = 0 ; i < m_ndim ; ++i) {
      ...
      if ( d < m_nnu ) b(m,i) = m_Auu(d      ) * x(m,i);
      else             b(m,i) = m_App(d-m_nnu) * x(m,i);
    }
  }
}
...
inline xt::xtensor<double,2> MatrixDiagonalPartitioned::Dot(
...
inline xt::xtensor<double,1> MatrixDiagonalPartitioned::Dot(
...
inline xt::xtensor<double,2> MatrixDiagonalPartitioned::Solve(
  const xt::xtensor<double,2> &b,
  const xt::xtensor<double,2> &x)
{
