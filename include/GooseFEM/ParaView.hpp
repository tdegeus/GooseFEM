...
inline std::string join(const std::vector<std::string>& lines, const std::string &sep)
{
  ...
  for (auto line: lines)
  {
    ...
    if      (line[0]           == sep[0]) out += line;
    else if (out[out.size()-1] == sep[0]) out += line;
    else                                  out += sep + line;
  }
...
inline std::vector<std::string> Connectivity::xdmf(size_t n_indent) const
{
  ...
  out.push_back(
    "<Topology NumberOfElements=\"" + std::to_string(m_shape[0]) +
    "\" TopologyType=\"" + to_string(m_type) + "\">");
...
inline std::vector<std::string> Attribute::xdmf(size_t n_indent) const
{
  ...
  {
    out.push_back(
      "<Attribute AttributeType=\"Scalar\" Center=\"" + to_string(m_type) +
      "\" Name=\"" + m_name + "\">");

    out.push_back(indent(n_indent) +
      "<DataItem Dimensions=\"" + std::to_string(m_shape[0]) +
      "\" Format=\"HDF\">" + m_fname + ":" + m_dataset + "</DataItem>");
  }
  else if (m_shape.size() == 2)
  {
    out.push_back(
      "<Attribute AttributeType=\"Vector\" Center=\"" + to_string(m_type) +
      "\" Name=\"" + m_name + "\">");
...
inline std::vector<std::string> TimeSeries::xdmf(size_t n_indent) const
{
  ...
  {
    out.push_back(
      "<Grid Name=\"Increment " + std::to_string(inc) + "\">");

    out.push_back(indent(n_indent) +
      "<Time Value=\"" + std::to_string(inc) + "\"/>");
...
inline void TimeSeries::write(const std::string& fname, size_t n_indent) const
{
  ...
  myfile << indent(n_indent) +
    "<Domain>" << std::endl;

  myfile << indent(n_indent * 2) +
    "<Grid CollectionType=\"Temporal\" GridType=\"Collection\" Name=\"TimeSeries\">" << std::endl;
