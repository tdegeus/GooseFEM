...
TEST_CASE("GooseFEM::Iterate", "Iterate.h")
{
    ...
    SECTION("StopList::stop - unsorted input")
    {
        ...
        for (size_t i = 0; i < res.size(); ++i) {
            REQUIRE(a.stop(res[i], 1e-3) == conv[i]);
            REQUIRE(b.stop_sorted(res[i], 1e-3) == false);
        }
    }

    SECTION("StopList::stop_sorted - sorted input")
    {
        GooseFEM::Iterate::StopList stop(5);
        ...
        for (size_t i = 0; i < res.size(); ++i) {
            REQUIRE(stop.stop_sorted(res[i], 1e-3) == conv[i]);
        }
    }

    SECTION("StopList::stop_sorted - unsorted input")
    {
        GooseFEM::Iterate::StopList stop(5);
