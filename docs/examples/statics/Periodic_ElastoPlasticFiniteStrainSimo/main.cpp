    ...
    for (size_t inc = 0; inc < dgamma.size(); ++inc) {
        ...
        xdmf.push_back(
            {PV::Topology(file, "/conn", mesh.getElementType()),
             ...
             PV::Attribute(
                 file, "/disp/" + std::to_string(inc), PV::AttributeCenter::Node, "Displacement"),
             PV::Attribute(
                 file, "/sigeq/" + std::to_string(inc), PV::AttributeCenter::Cell, "Eq. stress"),
             PV::Attribute(
                 file, "/epseq/" + std::to_string(inc), PV::AttributeCenter::Cell, "Eq. strain")});
    }
