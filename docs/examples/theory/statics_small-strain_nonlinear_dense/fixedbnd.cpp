...
int main()
{
  ...
  GooseMat::Cartesian3d::Material mat = GooseMat::Cartesian3d::Material(1.0,0.5,0.1,10.0);
  ...
  for ( size_t iiter = 0 ; iiter < 10 ; ++iiter )
  {
    ...
    if ( iiter == 0 ) DU_u = K_uu.ldlt().solve( T_u - F_u - K_up * DU_p );
    else              DU_u = K_uu.ldlt().solve( T_u - F_u               );

    // add displacement update to global system
    for ( size_t i = 0 ; i < ndof ; ++i ) {
      if      ( dofs(i) < nnu ) u ( i ) += DU_u ( dofs(i)       );
      else if ( iiter == 0    ) u ( i ) += DU_p ( dofs(i) - nnu );
    }
