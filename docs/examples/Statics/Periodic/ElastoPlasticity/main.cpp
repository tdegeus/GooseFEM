...
int main()
{
  ...
  GMatElastoPlastic::Cartesian3d::Matrix mat(nelem, nip);
  ...
  for (size_t inc = 0; inc < dgamma.size(); ++inc)
  {
    ...
    sigeq(inc) = GMatElastoPlastic::Cartesian3d::Sigeq(Sigbar);
    epseq(inc) = GMatElastoPlastic::Cartesian3d::Epseq(Epsbar);
  }
